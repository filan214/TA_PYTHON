...
- ✅ **Tahap 9 (DSS Dashboard) + `src/run_all.py` SELESAI.** `app/dashboard.py` (Streamlit, tiga subsistem DSS: Data/Model/UI-saran) memuat artefak Tahap 6–8, memilih model terbaik (RF·gt) dari `metrics_summary.csv`, dan memberi rekomendasi order-up-to bahasa awam ("Stok kini X; ROP=Y → SARAN: pesan Z unit") dgn indikator warna (🟢 aman / 🟡 mendekati ROP / 🔴 di bawah ROP) + grafik aktual-vs-ramalan tervalidasi + input stok manual. Logika keputusan diekstrak ke `src/dss/recommend.py` (teruji, `tests/test_dss.py`), dashboard = lapisan tipis. `src/run_all.py` mengorkestrasi Tahap 1→8 (skip artefak kecuali `--force`). Semua diverifikasi: **suite penuh 137 hijau** (termasuk smoke-test headless AppTest yang mem-boot dashboard tanpa error). Streamlit ditambahkan/dipasang (sudah di `requirements.txt`).
- 📌 **KEPUTUSAN DESAIN D12 (DSS forecast = load-only proxy, disepakati dgn pembimbing/user):** dashboard **secara sengaja TIDAK melatih ulang model / tidak melakukan forecast live.** Angka "ramalan" yang ditampilkan adalah **prediksi one-step walk-forward TERVALIDASI TERAKHIR** dari Tahap 7 (RF·gt) — diberi **label jujur** minggu yang diramal + tanggal *as-of* data (mis. "Ramalan untuk minggu 30 Jun 2025, berdasar data s.d. 23 Jun 2025"), **bukan** diklaim sebagai "ramalan minggu depan" yang hidup. Alasan: konsisten dgn lingkup **prototipe** (tanpa integrasi POS real-time, tanpa retraining otomatis) — tercatat sbg batasan di Bab I/V. Konsekuensi: nol pelanggaran kontrak "jangan latih ulang artefak Tahap 6"; angka yang ditampilkan sepenuhnya dapat ditelusuri ke artefak evaluasi.
- 🎯 **SELURUH PIPELINE (Tahap 1→9) SELESAI & terverifikasi.**
...
11. [x] Tahap 9 (Streamlit DSS) → `app/dashboard.py` + logika teruji `src/dss/recommend.py` (`tests/test_dss.py`) + smoke-test headless `tests/test_dashboard_smoke.py` (AppTest, boot tanpa error). Tiga subsistem (Data/Model/UI), input stok manual → saran order-up-to, indikator warna, grafik aktual-vs-ramalan. **Keputusan desain terdokumentasi (D12, lih. bawah).**
12. [x] `src/run_all.py` end-to-end (Tahap 1→8, skip artefak kecuali `--force`, `--from/--to-stage`, `--no-trends`) + `tests/test_run_all.py` hijau + `README` cara menjalankan.
13. [x] `pytest -q` seluruhnya hijau — **137 uji lulus**.
